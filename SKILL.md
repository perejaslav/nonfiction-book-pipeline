...
**Два способа включить оглавление:** эквивалентны — выбрать один.
- Флаг `--toc` в командной строке pandoc (запись в `pandoc ... --toc`).
- Параметр `toc: true` в YAML frontmatter (запись в manuscript.md). При сборке через YAML frontmatter **всегда** указывать `toc: true` — иначе pandoc не сгенерирует оглавление при использовании XeLaTeX. Пример: см. `references/pandoc-pdf-epub-export.md`.

**CRITICAL (из практики)**: Оглавление в PDF должно быть **кликабельным** — ссылки ведут на страницы глав. Для этого:
1. В YAML frontmatter: `toc: true` (не просто `--toc` в команде).
2. После сборки body.pdf — убедиться, что страница 2 содержит оглавление, а не сразу текст.
3. Если страница 2 — пустая или с текстом главы — frontmatter не сработал, проверить наличие `toc: true`.

**Проверить результат:**
```bash
pdfinfo book.pdf        # страницы, размер, метаданные
pdftotext book.pdf - | head -60  # читаемость: обложка → оглавление → контент
pdftotext -f 1 -l 1 book.pdf -  # страница 1 = обложка
pdftotext -f 2 -l 2 book.pdf -  # страница 2 = оглавление (кликабельное)
```
...
1. Страница 1 — обложка (cover-single.pdf, полностраничное изображение)
2. Страница 2 — оглавление (toc: true в frontmatter, кликабельное)
3. Страница 3+ — Введение, главы, Заключение
...
**Важно**: не писать вручную `# Оглавление` или `# Содержание` в теле manuscript.md — pandoc генерирует оглавление автоматически из `toc: true` в frontmatter и `toc-title` как заголовок.
