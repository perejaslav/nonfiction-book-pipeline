...
| `に` (яп. частица) | удалить или заменить на русский предлог |
| `复制` (копировали) | `копировали` |
| Любые `\u4e00-\u9fff` | Перевести на русский |

**Критический фейл-мод:** При expansion русского текста модель может вставить `大多数` или `同時` без запроса. Это систематический баг — лечится простой заменой + повторной проверкой всей главы.
